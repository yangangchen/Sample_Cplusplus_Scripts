...
// Must cast the types T1 & T2 to the type Eigen::MatrixXd using X.template cast<double>() & y.template cast<double>()
template<typename T1, typename T2>
double Dataset<T1,T2>::cross_validation (Regressor* model, const int K) {
    Eigen::VectorXd cv_errors(K);
    for (int k=0; k<K; k++) {
        Kfoldcv_split(K,k);
        model -> train(X_Kfoldcv_train.template cast<double>(),
                       y_Kfoldcv_train.template cast<double>());
        cv_errors(k) = model -> evaluate_error(X_Kfoldcv_cv.template cast<double>(),
                                               y_Kfoldcv_cv.template cast<double>());
    }
...
template<typename T1, typename T2>
double Dataset<T1,T2>::train_test_evaluation (Regressor* model) const {
    model -> train(X_train.template cast<double>(),
                   y_train.template cast<double>());
    return model -> evaluate_error(X_test.template cast<double>(),
                                   y_test.template cast<double>());
}
